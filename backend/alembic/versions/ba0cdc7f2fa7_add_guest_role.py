...
def upgrade() -> None:
    op.execute("COMMIT")
    op.execute(
        "ALTER TYPE organization_role ADD VALUE IF NOT EXISTS 'GUEST'"
    )
    op.execute(
        "ALTER TYPE invitation_role ADD VALUE IF NOT EXISTS 'GUEST'"
    )
    op.execute(
        "ALTER TYPE notification_type "
        "ADD VALUE IF NOT EXISTS 'ORGANIZATION_INVITATION'"
    )
